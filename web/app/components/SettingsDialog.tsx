...
export default function SettingsDialog({ isOpen, onClose }: Props) {
  ...
  const stockDerivedTypes = Array.from(
    new Set((stockRows as any[]).map((r) => r.item_properties?.equipment_type).filter(Boolean))
  ) as string[];
        ...
        <div className="flex flex-col gap-1 w-36 shrink-0 border-r border-neutral-100 pr-4">
          {isAdmin && (
            <button
              ...
              className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-semibold text-left transition-all ${section === "users" ? "bg-neutral-100 text-neutral-800" : "text-neutral-400 hover:text-neutral-600 hover:bg-neutral-50"}`}
            >
              <Users size={13} /> Users
            </button>
          )}
          <button
            ...
            className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-semibold text-left transition-all ${section === "equipment" ? "bg-neutral-100 text-neutral-800" : "text-neutral-400 hover:text-neutral-600 hover:bg-neutral-50"}`}
          >
          ...
          {section === "users" && isAdmin && (
            <div className="flex flex-col flex-1 gap-3">
              ...
              <Button
                ...
                className="w-full h-9 text-xs gap-1.5 bg-neutral-900 hover:bg-neutral-700 transition-all hover:cursor-pointer hover:scale-103"
              >
                <Save size={12} />
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                {equipmentTypes.map((type) => (
                  <div key={type} className="flex items-center justify-between px-3 py-2.5 rounded-xl border border-neutral-100 bg-neutral-50">
                    {editingType === type ? (
                      <Input
                        value={editingValue}
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      {editingType === type ? (
                        <>
                          <button onClick={() => handleRenameType(type)} className="p-1.5 rounded-lg text-green-500 hover:bg-green-50 transition-colors"><Check size={13} /></button>
                          <button onClick={() => setEditingType(null)} className="p-1.5 rounded-lg text-neutral-400 hover:bg-neutral-100 transition-colors"><X size={13} /></button>
                        </>
                      ) : (
                        <>
                          <button onClick={() => { setEditingType(type); setEditingValue(type); }} className="p-1.5 rounded-lg text-neutral-400 hover:text-neutral-700 hover:bg-neutral-100 transition-colors hover:scale-103"><Pencil size={13} /></button>
                          <button onClick={() => handleDeleteType(type)} className="p-1.5 rounded-lg text-neutral-400 hover:text-red-500 hover:bg-red-50 transition-colors hover:cursor-pointer"><Trash2 size={13} /></button>
                        </>
                      )}
                    </div>
                  </div>
              ...
              <div className="flex gap-2 pt-1">
                <Input value={newType} onChange={(e) => setNewType(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") handleAddType(); }} placeholder="New equipment type..." className="h-8 text-xs" />
                <Button type="button" onClick={handleAddType} variant="ghost" className="h-8 px-3 text-xs gap-1 border border-neutral-200 transition-all hover:scale-103 hover:cursor-pointer"><Plus size={12} /> Add</Button>
              </div>
            </div>
          )}
        </div>

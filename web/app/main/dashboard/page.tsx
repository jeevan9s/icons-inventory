...
export default function Dashboard() {
  ...
  const addButtonText =
    tab === "inventory" ? "Add Item" : tab === "loans" ? "Log Rental" : "";
  ...
  const lowStock = inventoryData.filter(
    (i: any) => i.status === "Low Stock" || i.status === "Out of Stock",
  ).length;

  const {Dialog: Export, open: ExportOpen} = useDialog(ExportDialog); 
          ...
          <div className="flex flex-col min-w-0 xl:flex-[2] border border-neutral-100 rounded-2xl overflow-hidden bg-white min-h-[500px] xl:min-h-0">
            <div className="flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 border-b border-neutral-100 gap-3">
              ...
              <div className="flex flex-row gap-2">
                ...
                <button onClick={ExportOpen} className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-5 py-2 sm:py-2.5 bg-neutral-800 text-white text-xs sm:text-sm rounded-xl hover:cursor-pointer hover:bg-neutral-700 hover:scale-105 transition-colors font-mp whitespace-nowrap">
                  <Download size={13} /> Export
                </button>

                <button onClick={AddOpen} className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-5 py-2 sm:py-2.5 bg-neutral-800 text-white text-xs sm:text-sm rounded-xl hover:cursor-pointer hover:bg-neutral-700 hover:scale-105 transition-colors font-mp whitespace-nowrap">
                  <Plus size={13} /> {addButtonText}
